...
"""
    通用代码：针对列表的自定义工具
    函数式编程：参照微软公司的Linq框架，自定义了集成操作框架
    自定义集成操作框架功能更强大，便于调试；内置高阶函数性能更高,知名度更大。
"""
...
class IterableHelper:
    ...
    @staticmethod
    def get_max(target: list, func_condition):
        """
    ...
    @staticmethod
    def get_min(target: list, func_condition):
        """
            获取指定条件的最小对象(第一个) max青春版

        :param target: 列表
        :param func_condition: 条件: 函数/方法类型参数 可写lambda函数 lambda item: item.hp
        :return: 返回指定条件的最小对象(第一个)
        """
        temp = target[0]
        for i in range(1, len(target)):
            if func_condition(target[i]) < func_condition(temp):
    ...
    @staticmethod
    def order_by(target: list, func_condition, descending: bool = False):
        """
            根据指定条件排列 sorted青春版

        :param descending: 默认False(升序)
        :param target: 列表
        :param func_condition: 条件: 函数/方法类型参数 可写lambda函数 lambda item: item.hp
        :return: None
        """
...
a1 = IterableHelper()
a2 = [3, 7, 5, 6, 1]
a2[:] = a1.order_by(a2, lambda a: a, True)
print(a2)
