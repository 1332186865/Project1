...
class Finding:
    def __init__(self):
        self.docx_dir = r"data/52插入语 全 去来源.txt"
        self.output_dir = r"data/53插入语 全 单独.txt"
    ...
    def run(self):
        ...
        with open(self.docx_dir, 'r', encoding='utf-8') as docx_file:
            for line in docx_file:
                ...
                line_temp_2 = re.match(p2, str(line))
                line_temp_3 = re.match(p3, str(line))
                self.count += 1
                print("\r", end="")
                ...
                if line_temp_1 is not None:
                    ...
                    with open(self.output_dir, 'a', encoding='utf-8') as out_file:
                        out_file.write(line)
                    continue
                if line_temp_2 is not None:
                    self.logger.info(f"正在写入{line.strip()}")
                    with open(self.output_dir, 'a', encoding='utf-8') as out_file:
                        out_file.write(line)
                    continue
                if line_temp_3 is not None:
                    self.logger.info(f"正在写入{line.strip()}")
                    with open(self.output_dir, 'a', encoding='utf-8') as out_file:
                        out_file.write(line)
                    continue
