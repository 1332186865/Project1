...
class Dict:  # 哈希表(除留余数法+拉链法)
    ...
    def search(self, k):  # 查找关键字k,成功时返回其地址,否则返回空
        ...
        while p is not None and p.key != k:  # 查找key为k的结点p
            p = p.next
        return p  # 返回p
    ...
    def delete(self, k):  # 删除关键字k
        d = hash(k) % self.cap
        if self.ha[d] is None: return
        if self.ha[d].next is None:  # ha[d]只有一个结点
            if self.ha[d].key == k:
                self.ha[d] = None
            return
        pre = self.ha[d]  # ha[d]有一个以上结点
        p = p.next
        while p is not None and p.key != k:
            pre = p  # pre和p同步后移
            p = p.next
        if p is not None:  # 找到关键字为k的结点p
