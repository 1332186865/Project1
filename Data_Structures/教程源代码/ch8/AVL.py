...
class AVLTree:  # AVL树类
    ...
    def _delete(self, p, k):
        ...
        if p.key == k:  # 找到关键字为k的结点p
            if p.lchild is None:  # 结点p只有右子树的情况
                return p.rchild  # 直接用右孩子替代结点p
            elif p.rchild is None:  # 结点p只有左子树的情况
                return p.lchild  # 直接用左孩子替代结点p
            else:  # 结点p同时有左右子树的情况
                if self.getht(p.lchild) > self.getht(p.rchild):  # 结点p的左子树较高
                    q = p.lchild
                    while q.rchild is not None:  # 在结点p的左子树中查找最大结点q
                        q = q.rchild
                    ...
                    q = p.rchild
                    while q.lchild is not None:  # 在结点p的右子树中查找最小结点q
                        q = q.lchild
                    p = self._delete(p, q.key)  # 删除结点q
                    p.key = q.key  # 用q结点值替代p结点值
                    p.data = q.data
    ...
    def _search(self, p, k):  # 被search方法调用
        if p is None: return None  # 空树返回Nonel
    ...
    def _DispAVL(self, p):  # 被DispAVL方法调用
        if p is not None:
            ...
            if p.lchild is not None or p.rchild is not None:
                print("(", end='')  # 有孩子结点时才输出“(”
