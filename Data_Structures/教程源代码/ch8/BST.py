...
class BSTClass:  # 二叉排序树类
    ...
    def _SearchBST(self, p, k):  # 被SearchBST方法调用
        if p is None: return None  # 空树返回null
    ...
    def DeleteNode(self, p, f, flag):  # 删除结点p（其双亲为f）
        if p.rchild is None:  # 结点p只有左孩子(含p为叶子的情况)
        ...
        elif p.lchild is None:  # 结点p只有右孩子
            if flag == -1:  # 结点p的双亲为空(p为根结点)
            ...
            if q.rchild is None:  # 若结点q没有右孩子
                p.key = q.key  # 将被删结点p的值用q的值替代
                p.data = q.data
                p.lchild = q.lchild  # 删除结点q
            else:  # 若结点q有右孩子
                while q.rchild is not None:  # 找到最右下结点q,其双亲结点为f1
                    f1 = q
                    q = q.rchild
                p.key = q.key  # 将被删结点p的值用q的值替代
                p.data = q.data
